...
const UnstyledDataTable: FC<Props> = ({
  id,
  className,
  fields,
  rows,
  ...
  const [sortedItem, setSortedItem] = React.useState<SortField | null>(() => {
    fields.forEach((f) => {
      // use label rather than id as only explorer fields have ids
      f.defaultSort = f.label === defaultSort || f.defaultSort;
    });
  ...
  const items = sortByField(
    filtered,
    sortedItem? sortedItem.reverseSort : false,
    [sortedItem as Field],
    false,
    disableSort=false
  );

  const doChange = (formState: FilterSelections) => {
    if (handleFilterChange) {
      handleFilterChange(formState);
    }
  };

  const handleChipRemove = (chips: string[], filterList: { [x: string]: { options: string[]; transformFunc?: ((option: any) => string) | undefined; }; }) => {
    const next = {
      ...filterState,
    };
    ...
    if (textFilters.length) {
      query["search"] = textFilters.join("_");
    } else if (query["search"]) {
      query = _.omit(query, "search");
    }
  ...
  const handleFilterSelect = (filters: any, formState: FilterSelections) => {
    ...
    <Flex wide tall column className={className}>
      ...
      <Flex wide tall>
        <TableView
          fields={fields}
          rows={items}
          defaultSortedField={sortedItem || undefined}
          id={id}
          hasCheckboxes={checkboxes}
