...
export const filterByStatusCallback = (v: { [x: string]: Condition[]; suspended: any; }) => {
...
export function filterRows<T>(rows: T[], filters: FilterConfig) {
  ...
  return _.filter(rows, (row) => {
    ...
    _.each(filters, (vals, category) => {
      ...
      // strings
      else value = (row as { [key: string]: any })[category];
...
export function parseFilterStateFromURL(search: string) {
  ...
  const state = { initialSelections: {} as { [key: string]: boolean }, textFilters: [] as string[] };
  if (query.filters) {
  ...
  if (query.search) {
    state.textFilters = query.search.split("_").filter((item: any) => item);
  }
  return state;
}
...
export const sortByField = (
  ...
  return _.orderBy(
    rows,
    sortFields.map((s) => {
      return s ? (s.sortValue || s.value) : null;
    }),
