...
function UserSettings({ className, darkModeEnabled = true }: Props) {
  ...
  const { userInfo, logOut } = React.useContext(Auth) as AuthContext;
  ...
  return (
    <Flex className={className} gap="8" align>
      ...
      <Tooltip title="Account settings" enterDelay={500} enterNextDelay={500}>
        <PersonButton
          onClick={(event: React.MouseEvent<HTMLButtonElement>) => handleClick(event as any)}
      ...
      <SettingsMenu
        ...
        onClose={handleClose}
        onClick={handleClose}
        transformOrigin={{ horizontal: 140, vertical: -10 }}
      >
