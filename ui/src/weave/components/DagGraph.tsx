...
function DagGraph({ className, nodes }: Props) {
  //zoom
  const defaultZoomPercent = 50; // Was 85 but this is "too" zoomed in
  ...
  const stratify = d3d.graphStratify();
  ...
  return (
    <Flex className={className} wide tall>
      ...
      <SliderFlex tall column align>
        <Slider
          onChange={(_, value: number | number[]) => setZoomPercent(Array.isArray(value) ? value[0] : value)}
